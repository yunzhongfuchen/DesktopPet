...
public:
    ...
    static bool excute(QString sql, QString connName, QString dbFile = "cloudmusic.db");
    //以connName连接对数据库dbFile执行sql语句，该sql语句有返回值
    static QSqlQuery* select(QString sql, QString connName, QString dbFile = "cloudmusic.db");
    //删除db文件dbFile的connName连接以及对应的锁
    static void destoryConn(QString connName,QString dbFile = "cloudmusic.db");
    //删除db文件dbFile的所有连接和锁
    static void destoryOneDBConn(QString dbFile = "cloudmusic.db");
    //删除所有db文件的所有连接和锁
